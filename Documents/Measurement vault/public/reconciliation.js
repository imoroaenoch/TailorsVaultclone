...
async function reconcileClients(businessId) {
    ...
    try {
        ...
        const toPush = localClients.filter(local => {
            ...
            // Push if: (1) not in remote AND unsynced, OR (2) unsynced (offline-created)
            return (!remoteIds.has(id) && !local.synced) || (local.synced === false || local.created_offline === true);
        });
        ...
        for (const local of toPush) {
            try {
                ...
                const { data, error } = await supabase
                    .from('clients')
                    .insert([{
                        id: local.server_id || local.id, // UUID generated ONCE, use existing
                        user_id: user.id,
